...
contract WorkingContract {
    ...
    modifier onlyAdminOrOrganiser(uint256 eventId) {
        require(
            ((msg.sender == owner) || (eventOrganisers[msg.sender])),
            "Only Admin/Event Organisers can perform this action"
        );
        require(
            ((msg.sender == owner) ||
                (events[eventId].eventOrganiser == msg.sender)),
            "Admin Or Event Organiser Who Own The Event Can Only Modify Events"
        );
        _;
    }
    ...
    modifier checkEventTicketBuyer(uint256 eventId, address _ticketHolder) {
        ...
        require(
            ticketBought,
            "You Can Only Cancel The Ticket You Purchased For The Event"
        );
        _;
    }
    ...
    modifier ticketsAvailable(uint256 eventId, uint256 ticketQuantity) {
        require(
            events[eventId].eventAvailableTickets >= ticketQuantity,
    ...
    function addOrganiser(address organiserAddress) external {
        require(
            msg.sender == owner,
            "Only Admin Can Add Organisers can perform this action"
        );
    ...
    function cancelEvent(
        ...
        for (uint256 i = 0; i < currentEvent.ticketsBought.length; i++) {
            ...
            uint256 refundAmount = currentEvent.eventTicketPrice *
                ticketHolder.purchasedTickets;
            // Check if the buyer has purchased tickets
            if (ticketHolder.purchasedTickets > 0) {
                // Refund the buyer
                payable(ticketBuyer).transfer(refundAmount);
                currentEvent.eventAvailableTickets += ticketHolder
                    .purchasedTickets;
    ...
    function startSellingTickets(
        ...
        emit EventUpdated(
            ...
            events[eventId].eventTicketPrice,
            EventStatus.Started
        );
    }

    function stopSellingTickets(
        uint256 eventId
    ) external onlyAdminOrOrganiser(eventId) {
        events[eventId].status = EventStatus.NotStarted;
        emit EventUpdated(
    ...
    function withdrawEventTicketMoney(
        uint256 eventId
    ) external onlyAdminOrOrganiser(eventId) eventExists(eventId) {
        Event storage currentEvent = events[eventId];
        require(
            currentEvent.status != EventStatus.Cancelled,
            "Event is already cancelled"
